...
`execution_profile` is optional Host-owned compatibility policy. When a Run contains a `compatibility` manifest, the CLI requires an exact `base` or `numpy-core` profile match and every declared import must appear in `allowed_imports`. Request mismatch exits `2` before artifact read. For an admitted request, the CLI reads the manifest's canonical sibling `import-inventory.json`, verifies artifact bytes, schema-v3 manifest, sidecar digest/content, profile, package set, filename, size, build identity and manifest SHA-256, then requires every Host-allowlisted root to appear in the target-Guest `find_spec` inventory. Only then does it freeze digests/inventory and construct the Factory. Schema-v2 manifests remain legacy-readable but cannot enable profile binding. Artifact, sidecar, inventory, or policy mismatch exits `2` before Factory/Broker/Guest construction. Legacy requests without a compatibility declaration remain accepted, but configured Host profile policy still requires the schema-v3 bundle. This is artifact-bound declaration admission, not source inference or import-execution proof; see [Execution profile admission](profile-admission.md).
