...
The active runtime-performance direction is [COW Python reactor performance-density](docs/plans/2026-07-27-cow-python-reactor-performance-density.md): fast Python execution, cheap deterministic reset, and high ready/active density are coupled product goals. The first Linux `cow-ready-single-use` level is implemented: an explicitly fixed-memory CPython-WASI artifact can publish one sealed prepared image and instantiate bounded, independently private, never-served slots from it. Prepared-image warmup is profile-scoped: a no-hook CPython-ready shard seals after runtime readiness, while the `numpy-ready-v1` profile imports NumPy before sealing so matching requests do not repeat the import. NumPy remains a manual `profile-candidate`, not a production-safe default. This is not served-slot restore or a general density/performance claim; fresh-instance remains the portable fail-closed baseline. See the [A/B/C lifecycle and direct E2E comparison](docs/reports/scheduler-experiment-results.md#8-python-and-numpy-request-lifecycle).
...
- [Host-governed COW scheduler experiment report](docs/reports/scheduler-experiment-results.md)
