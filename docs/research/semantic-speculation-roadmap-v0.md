...
**2026-08-21 prepared-data decision:** **Current:** a research-only `.npy v1` lane requires one exact target-Guest `numpy.load` candidate plus a separate Host-authored `sources.read` contract. Physical read, strict bounded Go decode and Run-private seal are separately observable. Linux can prepare the verified `numpy-core` package shard once, derive one fixed 8 MiB numeric ndarray image from that immutable parent, and serve fresh private-COW Guests without per-consumer body transfer. The public runtime seam accepts only the typed package and fixed-array operations; arbitrary Host Python is rejected. **Measured:** the fixed fanout probes establish numerical parity, private mutation isolation, copy/map accounting, cleanup and weaker-authority EAGER labeling. The Phase 6 checker reconstructs 162 coordinates and 54 aggregates from three raw trial pairs, but its critical path is a model derived from component durations and scheduled lead gaps rather than observed interval unions. **Rejected for promotion:** P3's scalar dynamic token is not cryptographically joined to the staged body/object consumed by its body-copy monkeypatch; COW preparation retains WASI clock/random imports; and Phase 6 does not independently observe the timing intervals needed for an economics gate. The apparent multi-consumer COW/data-local wins are research controls, not promotion evidence. **Deferred:** a typed object-bound Host materialization ABI, observed interval capture, the 64/256 MiB extension, expensive real-loader fixtures, cross-Run retention and P8 single-flight. Production optimizer/API work, generic cache/object-store behavior, arbitrary heap snapshots, generic loaders and universal speedup claims remain rejected.
