...
func TestDecodeRunRequestAcceptsBoundedTypedRequirements(t *testing.T) {
	request, err := DecodeRunRequest([]byte(`{"run_id":"run-1","code":"result=1","inputs":{},"requirements":["browser","posix"]}`))
	if err != nil {
		t.Fatal(err)
	}
	if len(request.Requirements) != 2 || request.Requirements[0] != RequiredFeatureBrowser || request.Requirements[1] != RequiredFeaturePOSIX {
		t.Fatalf("requirements=%v", request.Requirements)
	}

	for name, raw := range map[string]string{
		"unknown":       `{"run_id":"run-1","code":"result=1","inputs":{},"requirements":["gpu"]}`,
		"duplicate":     `{"run_id":"run-1","code":"result=1","inputs":{},"requirements":["browser","browser"]}`,
		"null":          `{"run_id":"run-1","code":"result=1","inputs":{},"requirements":null}`,
		"duplicate key": `{"run_id":"run-1","run_id":"run-2","code":"result=1","inputs":{}}`,
		"too many":      `{"run_id":"run-1","code":"result=1","inputs":{},"requirements":["browser","daemon","dynamic_package_install","native_extension","native_threads","posix","shell","subprocess","browser"]}`,
	} {
...
func TestAdmitRunRequirementsReturnsTypedUnsupportedError(t *testing.T) {
	request := RunRequest{Requirements: []RequiredFeature{RequiredFeaturePOSIX, RequiredFeatureBrowser}}
	err := AdmitRunRequirements(request)
	var unsupported *UnsupportedRunError
	if !errors.As(err, &unsupported) {
		t.Fatalf("error=%v", err)
	}
	if unsupported.Code != OutcomeRuntimeUnsupported || len(unsupported.RequiredFeatures) != 2 || unsupported.RequiredFeatures[0] != RequiredFeatureBrowser || unsupported.RequiredFeatures[1] != RequiredFeaturePOSIX {
