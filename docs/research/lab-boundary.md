...
`research/labview` owns the latest-only `pysolate.lab-latest.v2` mechanism projection and `pysolate.lab-task.v2` debugger projection. The mechanism projector remains body-safe. The debugger projector pins a dedicated public release-readiness corpus and a real Guest report, creates and validates a private `ProfileExperimentFull` trajectory in `labstore`, and publishes only explicitly public source/output/workspace bodies whose digests match recorded events. The Web renderer restores the historical execution-list/selected-operation Inspector and exposes Timeline as a separate read-only surface. Neither surface can execute, replay, branch, grant authority, dispatch effects, or resolve arbitrary private bodies; replay and branch remain Host-owned runtime operations over the private store.

The Web renderer shows eight code-and-execution mechanism views, the restored Inspector, and a separate Timeline. Correctness bookkeeping and evidence identities remain machine validation rather than visible accomplishments. The renderer cannot infer new facts, execute a program, dispatch an effect, resolve non-public bodies or grant authority.
