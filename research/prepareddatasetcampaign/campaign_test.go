...
func TestBuildCampaignFailsClosed(t *testing.T) {
	...
	cases := map[string]func(*Manifest, []Trial){
		"manifest":        func(m *Manifest, _ []Trial) { m.Trials = 2 },
		"preregistration": func(m *Manifest, _ []Trial) { m.PreregistrationSHA256 = "sha256:drift" },
		"source":          func(m *Manifest, _ []Trial) { m.SourceCommit = "drift" },
		"duplicate":       func(_ *Manifest, t []Trial) { t[1].ID = 1 },
		"artifact":        func(_ *Manifest, t []Trial) { t[1].Eager.ArtifactSHA256 = "drift" },
		"parity":          func(_ *Manifest, t []Trial) { t[2].Fanout.Records[1].Parity = false },
	}
...
func eagerForTest(trial int) EagerReport {
	r := EagerReport{SchemaVersion: EagerSchemaVersion, ArtifactSHA256: frozenArtifactSHA256, FixtureBodySHA256: researchdata.CanonicalBodySHA256, HostReadNanos: uint64(10 + trial), HostDecodeNanos: uint64(20 + trial), ShardPrepareNanos: 100, WarmupFreshGuests: 1}
