...
func TestStoreCreateReopenAndPendingReplay(t *testing.T) {
	...
	call := LoggedCall{Sequence: 0, CallID: "call-1", Capability: "tools.echo", Arguments: json.RawMessage(`{"x":1}`)}
...
func TestStoreWaitDecisionConflictAndTerminalRunDoesNotReactivate(t *testing.T) {
	...
	call := LoggedCall{Sequence: 0, CallID: "call-1", Capability: "tools.wait", Arguments: json.RawMessage(`{"request":"x"}`)}
...
func TestStoreLogicalLimitRejectsWithoutTruncation(t *testing.T) {
	...
	call := LoggedCall{Sequence: 0, CallID: "call", Capability: "tools.echo", Arguments: json.RawMessage(`{}`)}
	if _, _, err := store.BeginCall(context.Background(), definition.ID, call); err != nil {
		t.Fatal(err)
	}
...
func TestStorePayloadSizeCountsUTF8BytesInSQL(t *testing.T) {
	...
	call := LoggedCall{Sequence: 0, CallID: "呼叫", Capability: "工具", Arguments: json.RawMessage(`{"参数":"值"}`)}
	if _, _, err := store.BeginCall(context.Background(), definition.ID, call); err != nil {
		t.Fatal(err)
	}
...
func TestStoreCancellationDoesNotReopenOrOverwriteTerminalRun(t *testing.T) {
	...
	call := LoggedCall{Sequence: 0, CallID: "call", Capability: "tools.wait", Arguments: json.RawMessage(`{}`)}
	if _, _, err := store.BeginCall(context.Background(), definition.ID, call); err != nil {
		t.Fatal(err)
	}
