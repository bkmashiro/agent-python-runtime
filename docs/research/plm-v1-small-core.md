...
If the candidate and temporal proof are valid, `L` adopts it. Otherwise `L` rejects it and starts the canonical operation through the same Broker call. A typed uncertain provider outcome is materialised once as `provider_outcome_uncertain`; it is not replayed. V1 immediately materialises at the same source point, so `L=M`.
...
**Invalid restart.** A binding, temporal, authority or provider-proof failure detaches the candidate before the Broker executes the canonical handler. The one Broker call therefore has baseline call order and one receipt.
...
| no unsafe replay | typed uncertain outcome and no-replay test |
