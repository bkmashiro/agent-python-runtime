...
**Current artifact-bound profile, Static Import Agent Code, and immutable RunPlan:** an untrusted `compatibility` manifest names `base` or `numpy-core` and declares a root-only import set. Host policy freezes allowed roots. The schema-v4 producer emits exact target-Guest discoverability and curated fresh-Guest import/operation qualification sidecars; the canonical verifier binds both sidecars, artifact, manifest, profile, and package identity. For profile-qualified execution, Agent-authored imports must form one contiguous module-level absolute preamble, and its AST roots must equal the caller declaration exactly. `static-agent-imports-v2` rejects obvious drift early. A fresh or never-served exact Guest then parses and compiles the source, executes only the preamble, retains aliases, seals the loaded module set, and removes import/eval/exec entrypoints from the business body's builtins. The Host validates the exact Guest sidecar and freezes an artifact/profile/policy-bound RunPlan before workspace activation or Broker construction. The pinned CPython import path emits a Pysolate audit event before module-cache lookup; a native pre-initialization hook rejects imports outside the sealed set and records bounded admitted/denied import receipts that the Host binds to the plan digest. Late denial fails the Run and never selects or migrates to a VM. A legacy no-manifest path remains only for internal artifact qualification and pre-profile ABI compatibility; it is not a profile-qualified placement claim. See [Execution profile admission](docs/profile-admission.md) and [Static Import Agent Code contract](docs/source-compatibility.md).
...
- [Conservative source import comparison](docs/source-compatibility.md)
