...
**Current artifact-bound profile admission:** an optional untrusted `compatibility` manifest names `base` or `numpy-core` and declares imports. Host policy freezes allowed import roots. The schema-v4 build pipeline first runs `importlib.find_spec` inside the exact target Guest and emits checksummed `import-inventory.json`; it then runs a curated set of imports and deterministic operations in separate fresh Guest invocations and emits `import-qualification.json`. The canonical verifier binds artifact, manifest, both sidecars, profile, and package identity before Factory/Guest work. Host `allowed_imports` must be a subset of the qualified roots, which must themselves be a subset of the discoverable roots. CLI, Hermes bridge, and Wazero reject identity, sidecar, catalog, or policy mismatches. Schema-v2/v3 bundles remain readable as legacy artifact identity, but cannot enable profile binding without qualification evidence. This is not arbitrary-Python source or transitive-closure proof: qualification applies only to the exact versioned probe set and does not prove arbitrary operations, dynamic imports, native behavior, or Run success. See [Execution profile admission](docs/profile-admission.md).
