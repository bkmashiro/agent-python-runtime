...
class GuestSourceContractTests(unittest.TestCase):
    ...
    def test_build_uses_official_pinned_inputs(self):
        ...
        self.assertIn("MERGE_DATA_SEGMENTS=${AGENT_RUNTIME_MERGE_DATA_SEGMENTS:-0}", text)
        self.assertIn("AGENT_RUNTIME_MERGE_DATA_SEGMENTS must be 0 or 1", text)
        self.assertIn("MERGE_DATA_SEGMENT_FLAGS=(-Wl,--merge-data-segments)", text)
        self.assertEqual(3, text.count('"${MERGE_DATA_SEGMENT_FLAGS[@]}"'))
