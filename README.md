...
`durable.NewExecutor(controller, durable.Limits{MaxActive: 2, MaxQueued: 16})` adds a bounded FIFO admission layer over the narrow `AttemptController` lifecycle interface. Prefer `Admit(ctx, runID)` and `Attempt.Result(ctx)`: completion and parking are explicit `AdvanceResult` states, while execution/control failures remain errors. Repeated admission of an already queued/active Run returns `ErrBusy`, and a full queue returns `ErrQueueFull`. `Submit`/`Wait` remain compatibility aliases; `Wait` converts a parked result back to `ErrParked`/`ParkError`.

An active Guest blocked in a Host tool keeps its slot. Only return from `Runner.Advance` releases it. A parked logical Run can outlive its Guest without holding an active slot. Persist a decision through `Runner.Decide`, then explicitly `Admit` the same Run again. If the previous attempt is still active, wait for its result and retry rather than silently dropping a wake-up. Admission and future scheduling stay outside `Runner`; the Runner continues to own journals, replay, effects, and Guest construction.
