...
The Host attaches a discard-only WASI stdout detector. Canonical Python `print` never reaches FD1. Any direct low-level write, such as `os.write(1, ...)`, marks the run invalid even when Guest code catches the write result. WASI stderr is separately capped at 16 KiB for Host diagnostics, is discarded on successful runs, and is never promoted to model logs, a terminal result, a receipt, or evaluation evidence.

The implementation follows the useful DeepSeek Harness Code Mode boundary: only outer logs and the completion value re-enter model context, while intermediate tool values remain execution-local. Reference: `deepseek-ai/deepseek-harness@47f943859bef60e4160492346772ded9b24f765a`, `packages/core/tools/README.md`, lines 118–125 and 158–163.
