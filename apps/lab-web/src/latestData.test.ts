...
describe('latest-only Lab snapshot', () => {
  it('loads eight evidence-bound mechanism examples without a cohort panel', async () => {
    ...
    expect(snapshot.demos[0].metrics.map((metric) => metric.value)).toContain('1.914×');
    expect(snapshot.demos[1].metrics.map((metric) => metric.value)).toEqual(['3149 ms', '2132 ms', '1017 ms']);
