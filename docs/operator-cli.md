...
`execution_profile` is optional Host-owned compatibility policy. When a Run contains a `compatibility` manifest, the CLI requires an exact `base` or `numpy-core` profile match and every declared import must appear in `allowed_imports`. `conservative-python-imports-v1` then compares obvious bounded static imports in exact request source against the declaration and profile. Undeclared/unqualified static roots, dynamic import/execution, relative imports, lexical ambiguity, or comparison bounds overflow exit `2` before artifact read, Factory construction, workspace, Broker, or Guest work. For an admitted request, the CLI reads the manifest's canonical sibling `import-inventory.json` and `import-qualification.json`, verifies the schema-v4 artifact/manifest/sidecar identity, requires every Host-allowlisted root to appear in the target-Guest qualified set and every qualified root to be discoverable, binds that exact profile into the `RunConfig` passed to the Factory/Runner, and repeats artifact-bound source comparison. The Runner defensively repeats admission. Schema-v2/v3 manifests remain legacy-readable but cannot enable profile binding. The immutable result follows `abi/v1/compatibility-result.schema.json`, includes `syntax_checked=false`, and does not grant imports or prove arbitrary Python behavior. Legacy requests without a compatibility declaration remain accepted. See [Execution profile admission](profile-admission.md) and [Conservative source import comparison](source-compatibility.md).
