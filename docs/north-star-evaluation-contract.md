...
A follow-up three-arm prompt-causality diagnostic isolated two recurrent Python failures (`rd-004` and `rd-006`) across six paired replicates per task. The compact control passed 7/12, an exact-required-call-plan instruction passed 9/12, and the same instruction plus authoritative initial CWD context passed 10/12. Nine of ten failures omitted the initial directory transition; one produced an exact output-format mismatch after safe zero-call repair. The direction supports a prompt/interface contribution, but exact paired McNemar tests were not significant (`p=0.6875`, `1.0`, and `0.25`), so this remains underpowered development evidence rather than a placement gate. The strict trace oracle remains unchanged because final-state equivalence alone does not prove effect equivalence without explicit dependency/commutativity metadata. Formal artifacts are under `eval/agentic/results/codex-spark-prompt-causality-2026-08-11/`.
