...
**2026-08-21 prepared-data decision:** **Current:** a research-only `.npy v1` lane joins one exact target-Guest `numpy.load` candidate to a separate Host-authored `sources.read` contract. The physical read, strict bounded decode, Run-private seal and fresh-Guest materialisation remain distinct from the logical claim, which occurs only after the exact dynamic token is consumed. Linux can prepare the NumPy package shard once, derive one fixed 8 MiB numeric ndarray image from that immutable parent, and serve fresh private-COW Guests without per-consumer body transfer. The runtime accepts only the exact NumPy package fragment and exact bounded ndarray fragment; arbitrary Host Python is rejected. **Measured:** `prepared-data-phase6-linux-v1.json` records 162/162 complete parity/cleanup coordinates over three preregistered trials. Private-COW and data-local paths beat serial load for two and four consumers at every registered lead gap. At one consumer, private-COW first wins at the registered 1000 ms gap; 1000 ms still leaves part of the approximately 2.3 s derivation on the critical path. EAGER-style schedule-local persistence wins multi-consumer latency while retaining weaker interpreter and authority isolation. The exact cheap recompute control remains fastest at 61.8, 123.6 and 245.8 ms median for one, two and four consumers. **Rejected for this prototype:** production optimizer/API work, generic cache/object-store behavior, arbitrary heap snapshots, generic loaders, and universal speedup claims. **Deferred:** the 64/256 MiB payload extension, a dedicated object-only remapped extent, expensive real-loader fixtures, cross-Run retention, and P8 single-flight. Those require a new preregistered campaign and an economic case that the 8 MiB core does not supply.
