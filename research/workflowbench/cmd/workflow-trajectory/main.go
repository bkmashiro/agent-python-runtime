...
func generate(evidencePath, storePath, outputPath, sourceCommit string) error {
	...
	for index, metrics := range evidence.Tasks {
		...
		if _, err = appendOne(bodyEvent(trajectory.EventAssistantReasoning, trajectory.SourceModel, actor, request.EventID, turnID, stepID, false, reasoning, labstore.KindProviderBody)); err != nil {
			return err
		}
		if _, err = appendOne(bodyEvent(trajectory.EventAssistantOutput, trajectory.SourceModel, actor, request.EventID, turnID, stepID, true, "I will invoke the sealed paired-treatment tool and inspect its measured Runtime evidence.", labstore.KindProviderBody)); err != nil {
			return err
		}
	...
	contextIDs := append([]string{system.EventID, developer.EventID, user.EventID}, resultIDs...)
	...
	if _, err = appendOne(bodyEvent(trajectory.EventAssistantReasoning, trajectory.SourceModel, actor, finalRequest.EventID, finalTurnID, finalStepID, false, "All task results are present in the exact final model context; summarize without inferring unmeasured effects.", labstore.KindProviderBody)); err != nil {
		return err
	}
	if _, err = appendOne(bodyEvent(trajectory.EventAssistantOutput, trajectory.SourceModel, actor, finalRequest.EventID, finalTurnID, finalStepID, false, summary, labstore.KindProviderBody)); err != nil {
		return err
	}
	if _, err = appendOne(trajectory.EventInput{Type: trajectory.EventTurnEnd, Source: trajectory.SourceHarness, ActorID: actor, ParentEventID: finalTurn.EventID, TurnID: finalTurnID, Status: "completed"}); err != nil {
		return err
	}
	if _, err = appendOne(trajectory.EventInput{Type: trajectory.EventSessionEnd, Source: trajectory.SourceHarness, ActorID: actor, ParentEventID: session.EventID, Status: "completed"}); err != nil {
		return err
	}
