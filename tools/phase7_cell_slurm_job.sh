...
#SBATCH --job-name=pysolate-p7-cell
#SBATCH --export=NIL
#SBATCH --output=/vol/bitbucket/ys25/pysolate-p7-cell-%A_%a.out
...
test -n "${SLURM_ARRAY_JOB_ID:-}" && test -n "${SLURM_ARRAY_TASK_ID:-}"
[[ "$SLURM_ARRAY_JOB_ID" =~ ^[1-9][0-9]*$ ]]
[[ "$SLURM_ARRAY_TASK_ID" =~ ^(0|[1-9][0-9]*)$ ]]
[[ "${SLURM_JOB_ID:-}" =~ ^[1-9][0-9]*$ ]]
CELL_COUNT=$((7 * SAMPLES))
test "$SLURM_ARRAY_TASK_ID" -lt "$CELL_COUNT"
slots=(1 2 4 8 16 32 64)
SLOT_INDEX=$((SLURM_ARRAY_TASK_ID / SAMPLES))
REPEAT_INDEX=$((SLURM_ARRAY_TASK_ID % SAMPLES))
REQUESTED_SLOTS="${slots[$SLOT_INDEX]}"
CELL_TAG="${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}"

NODE_ROOT="${SLURM_TMPDIR:-/tmp}/pysolate-p7-cell-${CELL_TAG}"
INPUT="$NODE_ROOT/input"
REPO="$NODE_ROOT/repo"
RESULT="$NODE_ROOT/result"
OUTBOX="$STAGE/outbox"
ACK="$STAGE/ACK-${CELL_TAG}"
FAILED="$OUTBOX/FAILED-${CELL_TAG}"
FAILED_TMP="$OUTBOX/FAILED-${CELL_TAG}.partial"
OWNER_MARKER="$NODE_ROOT/.phase7-owner"
OWNER_MARKER_TMP="$NODE_ROOT/.phase7-owner.partial"
owner_token="${CELL_TAG}:${SOURCE_COMMIT}:$$"
failure_line=0
...
cmp -- "$INPUT/phase7_cell_slurm_job.sh" "$REPO/tools/phase7_cell_slurm_job.sh"
cd "$REPO"
...
export P7_ARM_ORDER="$ORDER"
"$INPUT/bin/apyrun-benchmark-linux-amd64" \
  -kind phase7-paired-density-cell \
  ...
  -samples "$SAMPLES" \
  -density-slots "$REQUESTED_SLOTS" \
  -density-repeat "$REPEAT_INDEX" \
  -arm-order "$ORDER" \
  -max-rss-bytes 8589934592 \
  -child-timeout 4m \
  -output "$RESULT/cell.json"
"$INPUT/bin/apyrun-benchmark-linux-amd64" \
  -kind validate-phase7-density-cell \
  -input "$RESULT/cell.json" \
  -schema "$REPO/benchmark/v1/phase7-paired-density-cell.schema.json" \
  -artifact "$INPUT/artifacts/agent-python-runtime-numpy-core.wasm" \
  -manifest "$INPUT/artifacts/manifest.json" \
  > "$RESULT/cell.json.validation.json"
(
  cd "$RESULT"
  sha256sum cell.json cell.json.validation.json > SHA256SUMS
  sha256sum --check SHA256SUMS
)
printf 'completed_utc=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$NODE_ROOT/ENVIRONMENT.txt"
printf '%s\n' "$SOURCE_COMMIT" > "$NODE_ROOT/RUN_COMPLETE"

archive_tmp="$OUTBOX/cell-${SLURM_ARRAY_TASK_ID}-result-${CELL_TAG}.tar.gz.partial"
archive="$OUTBOX/cell-${SLURM_ARRAY_TASK_ID}-result-${CELL_TAG}.tar.gz"
checksum_tmp="$OUTBOX/cell-${SLURM_ARRAY_TASK_ID}-result-${CELL_TAG}.tar.gz.sha256.partial"
checksum="$OUTBOX/cell-${SLURM_ARRAY_TASK_ID}-result-${CELL_TAG}.tar.gz.sha256"
ready_tmp="$OUTBOX/READY-${CELL_TAG}.partial"
ready="$OUTBOX/READY-${CELL_TAG}"
acked_tmp="$OUTBOX/ACKED-${CELL_TAG}.partial"
acked="$OUTBOX/ACKED-${CELL_TAG}"
