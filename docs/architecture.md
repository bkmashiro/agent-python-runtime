...
**Research-only** and makes no CPU/latency improvement claim. The dual-profile causal evidence
contract, real Runtime observation attachment and static read-only Lab inspector are
**Experimental**. Production capture is a strict rollback/reconciliation subset; full research
capture is private-by-default and bounded. Replay, rollback execution, resume and live tailing
remain Host integration concerns rather than Lab authority. See
[research/dual-profile-causal-evidence-v1.md](research/dual-profile-causal-evidence-v1.md).
...
The active tool surface uses one generic Guest-to-Host JSON call envelope and a small Host Registry. Each registration binds a canonical `CapabilitySpec`—capability/version identity, documentation, effect/playback declarations, handler identity, strict input/output schemas, Python projection and any optional bounded pre-dispatch qualification—to a Host handler, plus an opaque `CapabilityGrant` identity derived from the exact Host-owned per-Run policy. Before Guest startup, the Host seals the sorted specs, grants and total call budget into an immutable `pysolate.capability-plan.v7`; late registration is rejected. Handler identity remains stable implementation compatibility while changing target policy changes the grant and plan identities. The Broker accepts only that sealed plan, validates arguments before the handler, and validates results before returning them.

The optional `pysolate.plm-contract.v1` extension is also Plan-bound and default-off. It admits only direct typed reads with explicit temporal, provider non-interference, failure and authority rules. One Broker owns one Run-private candidate table; reservation separately bounds attempts, provider cost and worst-case result bytes. Strict modes revalidate Host evidence at the original logical call. `CURRENT` prepares transport only, `WALLCLOCK_OBSERVING` is not staged, and typed uncertain provider outcomes are materialized once without canonical replay. Snapshots separate logical receipts from provider starts, validation cost and private lifecycle events.

The CLI generates module objects and compatibility aliases from those same sealed specs:
...
**Experimental.** `pysolate.causal-evidence.v1` is the one current durable trajectory
contract. Canonical profile-independent event identities join Host execution attempts,
authority snapshots, typed effect transitions, workspace disposition, M1 source
occurrences/decisions, Runtime observations and three separate subagent inheritance planes.
The real Runtime adapter records receipt/source binding directly from Host-owned observations.

`production_rollback` is an allowlisted, body-free subset containing only facts needed for
safe replay prevention, cleanup, rollback or reconciliation. `experiment_full` is an opt-in,
private-by-default bounded trace. Portable projection removes body-only events; it does not
hide them in the UI. Ambiguous or non-compensable effects remain reconciliation-required and
are never called rolled back.

The append log and complete experiment bodies remain private `0600` state backed by typed
Labstore objects. The checked-in Lab data is generated by a named real CPython/WASI Guest run
and contains a body-safe experiment projection plus the strict production projection from the
same header and retained event identities. Lab is static and non-authoritative. See
[research/dual-profile-causal-evidence-v1.md](research/dual-profile-causal-evidence-v1.md).
...
For the PLM path, final sealed-source admission may prepare a Host-private candidate in the Run-owned `SplitPhaseTable`. The versioned `plm_capability_calls` pass runs inside the same exact final Guest and emits compiler-internal prepare plus original-position linearize/materialize calls. The derived program remains synchronous CPython and executes once. At the original call the Host rechecks call/source/argument/authority/provider identity and the operation's temporal contract; rejection starts the canonical operation, while typed uncertainty produces one terminal logical error without replay. No Python Future, Host dependency graph or arbitrary Broker injection is exposed. The predecessor `split_phase_capability_calls` issue/collect bridge is Removed. The old retained-prefix executor and independent semantic pre-dispatch controller have been removed. Current prefix admission uses the PLM owner described above; historical experiments require their original Git revision.
