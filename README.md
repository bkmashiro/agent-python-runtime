...
`Lease.Snapshot` produces a path-independent revision and a bounded file manifest; `workspace.Diff` reports deterministic added, modified and deleted metadata without embedding file contents. `Lease.Checkpoint` gives the harness a versioned local handoff token, and `Manager.AcquireCheckpoint` rejects a later attempt if the private bytes no longer match. The token is not a backup and writable bytes still remain outside durable Tool replay. `Lease.ExportChanges` can separately materialize a caller-bounded, versioned change bundle, and `CheckDirectoryConflicts` verifies only its touched Host paths against baseline metadata without writing them. Publication, merge and authorization remain outside the runtime. Python errors preserve private changes for Host inspection until cleanup. See [the workspace lifecycle and executable acceptance](docs/workspace.md), or run:
