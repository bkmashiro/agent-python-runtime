...
Full `/proc/self/smaps` collection is excluded from ordinary timed loads because page-table traversal over large mapping sets can dominate the workload being observed. Schema v8's `dirty-hold` and `mixed-v1` lanes are the explicit exceptions: they take three fixed-offset active snapshots while Guest bytearrays may remain live, include that diagnostic perturbation in measured time, and then record a separate post-refill final snapshot. CPU, timer-only, and heavy-tail lanes still make no in-load physical-memory claim.
