...
Full `/proc/self/smaps` collection is excluded from ordinary timed loads because page-table traversal over large mapping sets can dominate the workload being observed. Schema v9's `dirty-hold` and `mixed-v1` lanes are the explicit exceptions: they take three fixed-offset active snapshots while Guest bytearrays may remain live, include that diagnostic perturbation in measured time, and then record a separate post-refill final snapshot. CPU, timer-only, and heavy-tail lanes still make no in-load physical-memory claim.
...
`-pressure-workload cpu` keeps the tiny request compute-only. `-pressure-workload wasi-timer-wait -pressure-wait 100ms` adds a bounded Guest WASI timer wait so a consumer sweep can distinguish CPU-active work from wait-hiding concurrency. `-pressure-workload dirty-hold -pressure-dirty-bytes 16777216 -pressure-wait 2s` allocates a real Guest bytearray, writes one byte per 4 KiB page, and holds it across the three active snapshots. `mixed-v1` is a versioned deterministic 20-request cycle: 60% tiny CPU, 25% 50 ms timer wait, 10% 4 MiB dirty plus 500 ms hold, and 5% 16 MiB dirty plus 2 s hold. `heavy-tail-v1` is 95% tiny CPU plus 5% 2 s timer tail. Schema v9 records actual started/completed/failed counts for every request class, so the claimed distribution is auditable rather than inferred from the workload name. `-pressure-burst-factor 2|4|8` is valid only with `mixed-v1` and a load of at least ten seconds: baseline consumers run for the first half, then the remaining consumers are released together for one correlated closed-loop concurrency step. Evidence separately records pre-step and burst-window completed throughput. This is not an open-loop arrival trace, queueing model, or provider workload. None of these controlled workloads performs external network, filesystem, database, or provider I/O; they must not be reported as production network-I/O evidence.
