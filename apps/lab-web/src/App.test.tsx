...
describe('Pysolate Lab development debugger', () => {
  ...
  it('adds a complete clickable role-lane map to Timeline without removing the inspector', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'Fan-out, spans, and convergence' })).toBeVisible();
    ...
    expect(map.querySelectorAll('.lane-transition')).toHaveLength(18);
    expect(map.querySelectorAll('.lane-reuse')).toHaveLength(6);
    fireEvent.click(map.querySelector('.span-request')!);
    expect(map.querySelector('.lane-selection')?.textContent).toContain('physical request start → finish');
