...
export function validateDataset(value: unknown): LabDataset {
  ...
  if (data?.schema_version !== 'pysolate.lab-web-experiments.v1' || !/^sha256:[0-9a-f]{64}$/.test(data.report_sha256 ?? '') || !/^[0-9a-f]{40}$/.test(data.source_commit ?? '') || !/^sha256:[0-9a-f]{64}$/.test(data.corpus_sha256 ?? '') || !data.model || !data.study || !Array.isArray(data.runs)) throw new Error('Unsupported experiment dataset');
  for (const run of data.runs) {
    if (!run.run_id || !run.workload_id || !run.treatment || !['completed', 'failed', 'timed_out', 'unsupported'].includes(run.status) || !['passed', 'failed', 'not_run'].includes(run.oracle_status) || !Array.isArray(run.refs)) throw new Error('Invalid experiment run');
  }
...
export function ExperimentsView() {
  ...
  const runs = useMemo(() => dataset?.runs.filter((run) => (!workload || run.workload_id === workload) && (!treatment || run.treatment === treatment)) ?? [], [dataset, workload, treatment]);
  return <Stack className="experiments-view" gap="md">
    ...
    {dataset && <>
      ...
      <Paper withBorder><ScrollArea><Table striped highlightOnHover><Table.Thead><Table.Tr><Table.Th>Workload</Table.Th><Table.Th>Treatment</Table.Th><Table.Th>Recorded status</Table.Th><Table.Th>Oracle</Table.Th><Table.Th>Evidence</Table.Th><Table.Th>Recorded refs</Table.Th></Table.Tr></Table.Thead><Table.Tbody>{runs.map((run) => <Table.Tr key={run.run_id}><Table.Td><Code>{run.workload_id}</Code></Table.Td><Table.Td><Badge variant="light">{run.treatment}</Badge></Table.Td><Table.Td>{run.status}</Table.Td><Table.Td><Badge color={run.oracle_status === 'passed' ? 'teal' : run.oracle_status === 'not_run' ? 'gray' : 'red'}>{run.oracle_status}</Badge></Table.Td><Table.Td>{run.evidence_class}<br/><Text size="xs" c="dimmed">{run.evidence_completeness}</Text></Table.Td><Table.Td><RunRefs run={run} /></Table.Td></Table.Tr>)}</Table.Tbody></Table></ScrollArea></Paper>
    </>}
  </Stack>;
}
