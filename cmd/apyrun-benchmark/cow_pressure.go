...
func (evidence cowPressureEvidence) Validate() error {
	...
	for _, active := range evidence.LoadSamples[:len(evidence.LoadSamples)-1] {
		if active.Phase != "load-active" || active.Slots != lastSlots || active.RuntimeInstances != 1 ||
			...
			return fmt.Errorf("cow-pressure active-load identity drifted: phase=%q runtime_instances=%d mapping_name=%q mappings=%d mappings_valid=%t pool_valid=%t image_equal=%t slots=%d target=%d max=%d floor=%d critical=%d low=%d high=%d ready=%d leased=%d executing=%d waiting=%d queued=%d refilling=%d retiring=%d accounted=%d consecutive_failures=%d failures=%d breaker=%t", active.Phase, active.RuntimeInstances, active.COWMappings.Name, active.COWMappings.MappingCount, validPressureLoadMappingCount(active.COWMappings.MappingCount, lastSlots, evidence.Limits.Consumers), validPressureActivePoolState(active.Pool, lastSlots), active.PreparedImage == preparedImage, lastSlots, active.Pool.TargetCapacity, active.Pool.MaximumCapacity, active.Pool.Floor, active.Pool.Critical, active.Pool.Low, active.Pool.High, active.Pool.Ready, active.Pool.Leased, active.Pool.Executing, active.Pool.Waiting, active.Pool.Queued, active.Pool.Refilling, active.Pool.Retiring, active.Pool.SupplyAccounted, active.Pool.ConsecutiveFailures, active.Pool.TotalFailures, active.Pool.BreakerOpen)
		}
	}
...
func validPressureActivePoolState(state wazeroengine.PreparedPoolState, slots uint32) bool {
	return state.TargetCapacity == slots && state.MaximumCapacity >= slots && state.High == slots &&
		...
		state.SupplyAccounted == slots && state.Leased > 0 && state.Executing > 0 && state.Executing <= state.Leased &&
		state.Waiting == 0 && state.Retiring == 0 && state.ConsecutiveFailures == 0 && state.TotalFailures == 0 && !state.BreakerOpen
}
