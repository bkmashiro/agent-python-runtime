...
func TestCaptureRejectsSensitiveFieldsValuesLocatorsAndDuplicateKeys(t *testing.T) {
	observations := []readonlyrecorder.Observation{
		...
		{Source: readonlyrecorder.SourceWeb, Locator: "https://example.invalid?foo=1;token=hidden", Payload: []byte(`{"status":"ok"}`)},
		{Source: readonlyrecorder.SourceCLI, Locator: "curl https://example.invalid?foo=1;token=hidden", Payload: []byte(`{"status":"ok"}`)},
		{Source: readonlyrecorder.SourceCLI, Locator: "curl --url=https://example.invalid?%74oken=hidden", Payload: []byte(`{"status":"ok"}`)},
		{Source: readonlyrecorder.SourceCLI, Locator: "curl https://example.invalid?foo=1;%zz=hidden", Payload: []byte(`{"status":"ok"}`)},
		{Source: readonlyrecorder.SourceWeb, Locator: "https://example.invalid?%74oken=hidden", Payload: []byte(`{"status":"ok"}`)},
		{Source: readonlyrecorder.SourceWeb, Locator: "https://example.invalid?foo=1;%zz=hidden", Payload: []byte(`{"status":"ok"}`)},
		{Source: readonlyrecorder.SourceWeb, Locator: "https://user:hidden@example.invalid/status", Payload: []byte(`{"status":"ok"}`)},
		{Source: readonlyrecorder.SourceCLI, Locator: "curl https://user:hidden@example.invalid/status", Payload: []byte(`{"status":"ok"}`)},
		{Source: readonlyrecorder.SourceCLI, Locator: "curl --url=https://user:hidden@example.invalid/status", Payload: []byte(`{"status":"ok"}`)},
