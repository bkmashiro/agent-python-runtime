...
The later [production refactor](plm-v1-production-refactor.md) removed the structural duplicate work without adding another runtime. Under the same protocol it reduced the deltas to `+2.15%` cold and `+3.12%` precompiled. The original table remains the attributed Gate 6 decision input rather than being rewritten after the fact.
