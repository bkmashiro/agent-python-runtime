...
All 16 remediation cells completed. There were no unscorable or not-recorded rows, no raw-path collisions and no unknown provider counts. The run used 241 provider invocations. Strict reconstruction matched 36 logical READ calls to 36 physical Broker executions and 36 complete source/Plan/Grant/request-response/receipt joins. For every row, a separate fixed-revision oracle reloads the frozen task, validates a non-empty structured SimulationRun trajectory, recomputes the complete upstream `EvaluationType.ALL` RewardInfo, reconstructs the ActionEvaluator input from source-bound program events and compares both complete evaluator outputs. Default-reward mutation, diagnostic-reward mutation and empty-message controls are rejected. Duplicate-path and raw-body mutation controls were rejected. The body-safe aggregate is [tau2-t2-remediation-v1.json](../evidence/tau2-t2-remediation-v1.json), SHA-256 `cf4b1d0fc2cd3d7e1190843b609b822ec6fb2dbe62a49db9394b60c820729f85`.
