...
`execution_profile` is optional Host-owned compatibility policy. When a Run contains a `compatibility` manifest, the CLI requires an exact `base` or `numpy-core` profile match and every declared import must appear in `allowed_imports`. Request mismatch exits `2` before artifact read. For an admitted request, the CLI reads the manifest's canonical sibling `import-inventory.json` and `import-qualification.json`, verifies the schema-v4 artifact/manifest/sidecar identity, then requires every Host-allowlisted root to appear in the target-Guest qualified set and every qualified root to be discoverable. Only then does it freeze digests and both root sets and construct the Factory. Schema-v2/v3 manifests remain legacy-readable but cannot enable profile binding. Artifact, sidecar, catalog, or policy mismatch exits `2` before Factory/Broker/Guest construction. Legacy requests without a compatibility declaration remain accepted, but configured Host profile policy still requires a schema-v4 bundle. This is artifact-bound qualification for the versioned probe set, not source inference or transitive execution proof; see [Execution profile admission](profile-admission.md).
