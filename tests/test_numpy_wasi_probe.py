...
class NumPyWASIProbeContractTests(unittest.TestCase):
    def test_probe_uses_isolated_exact_sources_and_bounded_features(self):
        ...
        self.assertIn("resolve_wasm_extension_profile.py", script)
        self.assertIn("feature-profiles.json", script)
        self.assertIn("selection-report.json", script)
        self.assertIn("link-inputs.txt", script)
        self.assertNotIn("expected_static = {", script)
        ...
        self.assertIn('"feature_profile": selection["profile"]', script)
        self.assertIn('"selected_builtin_modules": [item["module"] for item in selection["modules"]]', script)
        link_source = LINK_PROBE.read_text()
        self.assertIn('#include "builtin-registry.h"', link_source)
        self.assertIn("register_selected_builtins()", link_source)
        self.assertNotIn("extern PyObject *PyInit_", link_source)
        self.assertNotIn("PyImport_AppendInittab", link_source)
    ...
    def test_workflow_is_manual_only_and_uploads_diagnostic_evidence(self):
        ...
        self.assertIn("runtime-report.json", text)
        self.assertIn("extension-profile/selection-report.json", text)
        self.assertNotIn("registration-report.json", text)
