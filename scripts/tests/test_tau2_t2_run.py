...
class Tau2T2RunTests(unittest.TestCase):
    def test_chunking_preserves_frozen_order(self):
        public = {"schema_version": runner.PUBLIC_SCHEMA, "tasks": [{"task_id": str(index)} for index in range(1, 17)]}
    ...
    def test_remediation_plan_is_treatment_only(self):
        public = {"schema_version": runner.REMEDIATION_PUBLIC_SCHEMA, "tasks": [{"task_id": str(index)} for index in range(1, 17)]}
        cells = runner.planned_cells(public)
        self.assertEqual(len(cells), 16)
        self.assertTrue(all(lane == "programmatic_python" and harness == "programmatic_python" for _, lane, harness in cells))
