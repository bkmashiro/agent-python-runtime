...
4. Reject normalized sensitive-key fragments—including prefixed forms such as `X-API-Key` and `aws_secret_access_key`—plus conservative secret-like values, URL userinfo passwords (including URLs embedded as CLI-command tokens), and parsed URL-query/CLI-argument locators such as `?auth=…` or `--client_secret …`.
