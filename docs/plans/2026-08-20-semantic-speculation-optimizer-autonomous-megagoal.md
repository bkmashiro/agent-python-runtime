...
- [x] Implement scratch-Guest execution and bounded capsule publication with typed terminal states.
- [x] Select original or derived program before final Guest execution; no racy runtime fallback.
- [x] Prove invalid suffix, cancellation, earlier exception and unclaimed capsule paths leave no logical region consumption or official workspace mutation.
- [x] Compare result, exception class, logical calls, authority state and traceback/source-location boundary against baseline.
- [x] Freeze an independent P5 mechanism/economics case matrix and preregistration before collecting any timing samples.
- [x] Freeze canonical body-free P5 trial records with overlapping-stage critical-path reconciliation and exact capsule/runtime lifecycle evidence.
- [x] Implement a one-shot monotonic stage recorder and portable peak-RSS observation that feed the frozen trial contract.
- [x] Add a private, never-served, single-use prepared scratch capacity, including mandatory Linux private-COW evidence and explicit consume/reject/discard lifecycle.
- [x] Implement the deterministic four-cell economics coordinate runner with sanitized execution input, frozen stage ordering, overlap-aware gap orchestration, failure teardown and canonical duplicate-safe JSONL emission.
- [x] Allow one exact Host-sealed capsule to be published into an empty run-private table after final capacity provisioning, without replacement or fallback.
- [x] Split target-Guest source validation/derived compile-load from exactly one formal execution on the same initialized final capacity.
- [x] Give the original treatment one equivalent initialized final capacity, including a mandatory private COW slot on Linux, with unchanged-source one-shot execution and no materialisation table.
- [x] Provision the bounded analyzer session into an initialized private never-served capacity before clock start, with no request-time fresh fallback and no Agent-source execution surface.
...
**2026-08-20 fresh scratch-Guest slice:** A separate one-shot authority-free Guest, never the bounded analyzer session, now validates decision-bound canonical live-ins and evaluates only the exact admitted RHS scalar tree. The v1 evaluator accepts `bool`/`int64`, bound names and `Add`/`Sub`/`Mult`; every intermediate must remain int64, preventing unbounded Python integer growth. It emits typed `ready`/`rejected`/`failed` terminal records, while Host cancellation emits `cancelled`; only `ready` can become a decision-bound, single-use capsule. Validation drift, unsafe AST, noncanonical live-ins and non-ready terminals publish nothing. Artifact `sha256:aae56baf0ea4b977cdc764e8246e7f358aea1ce3af87105e0f56c2af95af9c73` passed macOS and Linux Exact Guest execution/publication controls with one fresh module, no Broker and no workspace. Final original-versus-derived program selection remains absent, so P5 has not passed.

**2026-08-20 pre-execution selection slice:** Host now seals `pysolate.prepared-region-execution-selection.v1` over the exact decision, ready capsule, narrow patch, final source and derived AST before entering the final Guest. The final Guest first validates the unchanged original Run source, then independently reconstructs and validates the same one-RHS derived tree before compilation. `RunPreparedRegionDerived` is an experimental Engine-only entry point: it rejects Broker/workspace authority, validates the exact ready table entry without consuming it, and has no fallback or replay to the original program after commitment. Only the trusted helper claim consumes the capsule. Artifact `sha256:621f5fcec3f4bc7fc3550aa8fd1a275e7a6c09017518f535395c5bae84a297cb` matched baseline/derived result `42` on macOS and Linux Exact Guest runs; invalid suffix drift was rejected before execution with zero claims and zero consumption. This established deterministic selection and positive result parity; P5 remained open.

**2026-08-20 adversarial-parity slice:** The same immutable Exact Guest artifact passed baseline-versus-derived failure controls on macOS and Linux. An exception before the region preserved the exact `ValueError` message, logs and line-3 traceback while making zero claims and discarding the unclaimed capsule. An exception after the region preserved the exact `LookupError` message, logs and line-3 traceback after exactly one successful claim/consumption. A pre-cancelled final run produced no Guest response, made zero claims and discarded the capsule; the prior suffix-drift control likewise consumed nothing. Both lanes had zero capability calls, no Broker and no workspace, and the execution path contains no original replay after derived commitment. This closes the authored authority-free semantic parity checklist, but not timing economics or P5.

**2026-08-20 P5 campaign freeze:** Before collecting any P5 timing sample, the repository now pins an independent 11-case matrix (`sha256:e4025295cc47cdc62925f4a4e0b0d3f072726de9aff983c75a0b9187fd355cee`) and preregistration (`sha256:9db34a4fa8091bd9875132457dfcf9515fbf78802a5f0453029a4f52e1f776c6`) to the immutable P3/P4 lineages, implementation commit and Exact Guest artifact. One known pilot is excluded. Four positive scalar coordinates freeze add/multiply shapes, 0/250/1000/6000 ms finalization gaps, two profiles, two treatments and five seeded trials, yielding 80 timing records. Six controls freeze int64 overflow, unsafe RHS, suffix drift, pre/post-region exceptions and cancellation. Mechanism and economics remain separate: every exact control and parity boundary must pass, while each profile needs at least two positive coordinates with positive median critical-path saving and 4/5 positive trials. Failure records a no-go and retains original execution without transport or authority expansion. The independent reviewer reports `timing_samples_observed = 0`; P5 remains open until the frozen campaign runs and passes.

**2026-08-20 P5 trial-record contract:** `pysolate.semantic-speculation-phase5-trial.v1` now accepts only the 80 frozen economics coordinates and carries hashes rather than source, result, traceback or log bodies. It binds matrix, preregistration, harness, artifact, source and region identities; exact original/derived lifecycle counters; capsule byte/claim/consumption state; runtime-init and formal-Guest counts; memory/discard accounting; and authority/workspace terminal disposition. Twelve named stage intervals distinguish measured critical-path work, pre-clock treatment capacity and inapplicable work. Overlapping analysis/scratch/finalization intervals are unioned before an explicit unattributed remainder reconciles to the total critical path, avoiding double-counting hidden work. Cold and pre-provisioned provisioning dispositions, bounded capsule size, positive result identity and zero authority/call/orphan state all fail closed. This is contract-only evidence with no timing observation; the harness implementation and identity remain the next prerequisite before campaign execution.

**2026-08-20 P5 stage recorder:** A concurrency-safe, single-use `Phase5StageRecorder` now records the twelve frozen stages against an injected monotonic clock. It rejects unknown or duplicate stages, measured work before critical-path start, pre-clock work after start, active-stage finalization, stale tokens, reuse and clock regression. Finalization emits every stage in frozen order, marks absent work explicitly, unions overlapping critical intervals and reports only the exact unattributed remainder. Deterministic fake-clock tests cover hidden scratch work, pre-provisioned capacity, elapsed gaps and direct acceptance by the canonical trial decoder; targeted race tests pass. Peak RSS now uses `getrusage` with Linux KiB-to-byte normalization and Darwin byte semantics, with a bounded runtime fallback on other platforms. No campaign clock or Exact Guest trial was run; wiring the recorder to original/derived execution and freezing the resulting harness identity remain next.

**2026-08-20 prepared scratch capacity:** The pre-provisioned derived profile no longer depends on an unavailable scratch lifecycle. `PreparePreparedRegionScratch` now owns one initialized, authority-free, never-served Guest behind a single-use capacity. Non-Linux uses one private PreparedRuntime slot; Linux must select a private COW slot. `Execute` consumes the capacity before the attempt, performs no execution-time instantiate or `runtime_init`, and closes the module terminally. A second attempt is rejected; closing an unserved capacity records one discard. Capacity ownership holds an Engine lifecycle lease, and no fallback creates a fresh Guest if the requested prepared capacity is absent. The immutable selection artifact returned canonical `42` on macOS and Linux; Linux Exact Guest evidence explicitly required `COWHit=true`. This closes the pre-provisioned scratch-capacity prerequisite but records no P5 campaign timing. Evidence: `semantic-speculation-phase5-prepared-scratch-capacity-evidence-v1.json`.

**2026-08-20 deterministic coordinate runner:** `RunPhase5Coordinate` now owns the preregistered cold/pre-provisioned × original/derived lifecycle. Pre-provisioned original creates only final capacity; pre-provisioned derived creates analyzer, scratch and final capacities before the clock. Cold creates the same treatment-required topology after clock start. The finalization gap begins before provisioning/analysis so cold preparation and the derived analysis→patch→scratch→capsule pipeline can overlap it; final validation, derived compile/load and formal execution remain after the gap. The execution adapter receives only unchanged source, focus index and output name, never case/profile/treatment/trial labels or expected results. Any failed step drains the active gap and performs teardown without emitting a trial. Deterministic fake-clock tests assert exact four-cell call order, stage dispositions, canonical body-free decoding and duplicate-safe JSONL. All four records use synthetic clocks and adapters; the independent freeze reviewer still observes zero campaign timing samples. Actual Exact Guest adapter wiring and final harness identity remain open.

**2026-08-20 late capsule publication:** Pre-provisioning the final derived Guest exposed a real ordering conflict: its run-private materialisation table previously had to be constructed with a ready capsule, but the capsule does not exist until analysis and scratch execution after clock start. `PreparedRegionTable.Publish` now permits one valid Host-sealed decision/capsule pair to populate an empty or exactly matching unready entry after final capacity is ready. It cannot replace ready, consumed or discarded state; invalid, repeated or post-close publication rejects. Exact Guest tests provisioned final capacity against an empty table, observed no entry creation or claim, published later, then executed derived source with one claim and result `42`. macOS used PreparedRuntime; Linux required a private COW hit. This adds no Guest authority or generic transport and records no campaign timing. Evidence: `semantic-speculation-phase5-late-publication-evidence-v1.json`.

**2026-08-20 split final capacity:** The frozen stage protocol requires `final_patch_compile_load` and `final_execution` to describe actual work, but the previous `RunPreparedRegionDerived` combined target-Guest source validation, selection compile/load and `_run` in one call. `PreparePreparedRegionFinal` now provisions one initialized, never-served, single-use final Guest; on Linux it must acquire a private COW slot. Its capacity accepts one `Compile` attempt that revalidates the unchanged request and exact decision/capsule/patch/selection, calls target-Guest source validation and `runtime_select_prepared_region_execution`, and does not instantiate, call `runtime_init`, execute source or claim the capsule. `Execute` then accepts only the byte-identical request, invokes `_run` once, consumes the capsule once and closes terminally. Compile failure closes the capacity; second execution rejects; no fresh fallback exists. Exact macOS, race and Linux-COW tests preserve baseline result `42`, with zero compile/execution-time instantiations or runtime init calls. This makes the runner's two final stages physically measurable without moving execution into the compile stage. Evidence: `semantic-speculation-phase5-derived-final-capacity-evidence-v1.json`.

**2026-08-20 original final capacity:** Equivalent-capacity accounting now has an actual original-lane primitive rather than a request-time COW clone hidden inside `Engine.Run`. `PreparePreparedRegionFinal` may own an authority-free final Guest without a prepared-region table; `ExecuteOriginal` validates unchanged source and runs it exactly once. Provisioning owns all module instantiation, `_initialize` and `runtime_init` work, while execution reports one source validation and one formal Guest execution with zero runtime initialization. Linux must acquire a private COW slot before execution; macOS consumes one PreparedRuntime slot. The capacity closes after success or failure, rejects a second attempt and cannot enter derived compilation. Exact original/derived parity returned `42` on both platforms. Evidence: `semantic-speculation-phase5-original-final-capacity-evidence-v1.json`.

**2026-08-20 analyzer capacity:** `SemanticAnalysisSession.Prepare` now converts the bounded run-private analyzer session into an explicit initialized-but-never-served capacity before any analysis request. It requires PreparedRuntime, rejects workspace/Broker authority, accepts only one preparation, and fails closed if the underlying path reports a fresh fallback rather than a PreparedRuntime or Linux private-COW hit. `Run` remains disabled, so the prepared analyzer cannot execute Agent source. Exact macOS and Linux tests prepare first, observe positive provisioning/runtime-init counters and zero requests, reject repeat preparation and source execution, then retain patch-emission behavior. Evidence: `semantic-speculation-phase5-analyzer-capacity-evidence-v1.json`.

**2026-08-20 complete Exact Guest adapter and harness freeze:** The real operations adapter now executes the excluded pilot through independent analyzer, scratch and final capacities, exact analysis/patch identities, bounded scratch publication, one late Host publication, pre-start selection, split target-Guest compile/load and one final execution. Original and derived lanes match result and logs; derived compile claims nothing, final execution claims and consumes exactly once, and Linux requires private COW for all treatment capacities. The frozen trial semantics count scratch plus final as the two derived treatment Guest executions. `AggregatePhase5Campaign` independently requires all 80 canonical coordinates, exact uniqueness, matched outcome/authority parity, 4/5 positive savings and positive median saving per coordinate, and at least 2/4 passing coordinates in each profile; any failed mechanism or economics gate emits the frozen no-go action. The source-set reviewer pins adapter, recorder, trial, runner, runtime contracts and aggregate validator as harness `sha256:021ddcc7d76eebf1468174274f0b3c59c281625767235eca4a31046f4b6d2bb0` while reporting `timing_samples_observed = 0`. The next boundary is the complete 11-case Exact Guest mechanism-control gate; economics timing remains prohibited until it passes.

**Observed P5 mechanism result (2026-08-20): no-go.** The complete immutable 11-case matrix was exercised against the preregistered artifact before any economics timing observation. Six cases passed and five failed. The 256-multiply positive coordinate could not emit a valid target-Guest patch; the unchanged 512-add original source could not pass the artifact's recursive assigned-name collection, so baseline formal execution was unavailable. The suffix-drift, exception-before-region and pre-cancelled paths had correct rejection/exception/cancellation and zero-claim behavior, but the frozen adapter snapshot did not project the ready capsule's teardown discard, violating the exact lifecycle evidence gate. Because the matrix, preregistration and artifact are immutable, repairing the artifact or adapter would define a different post-remediation study rather than rescue this gate. The preregistered action is therefore `record_no_go_retain_original_execution_and_do_not_expand_transport_or_authority`; `phase5_gate_passed = false` and `timing_samples_observed = 0`. Do not run the 80-record economics campaign or continue into broader transport. Evidence: `semantic-speculation-phase5-mechanism-gate-evidence-v1.json`.
