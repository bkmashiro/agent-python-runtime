...
func TestGitChameleonNumPyDerivedManifestCoversDeclaredSubset(t *testing.T) {
	...
	for index, task := range manifest.Tasks {
		...
		if strings.Contains(task.Prefix+task.Suffix, "import ") || strings.Count(strings.TrimSpace(task.Prefix), "\n") != len(task.Inputs)-1 {
			t.Fatalf("source must use the preloaded NumPy shard and a call-only prefix example=%s", task.ExampleID)
		}
		for _, input := range task.Inputs {
			if input.Decoder != "int" && input.Decoder != "float" {
				t.Fatalf("invalid Host-input decoder example=%s input=%+v", task.ExampleID, input)
			}
			materialization := fmt.Sprintf(`%s = np.array([%s(_item) for _item in _host_%s.split(",")])`, input.Name, input.Decoder, input.Name)
			if !strings.Contains(task.Suffix, materialization) {
				t.Fatalf("materialization is not deferred to suffix example=%s input=%s", task.ExampleID, input.Name)
			}
		}
...
func newGitChameleonNumPyPreparedCapacity(ctx context.Context, artifact []byte, config runtimeconfig.RunConfig, maxConsumers uint32) (*plmPrefixPreparedCapacity, error) {
	capacity, err := newPLMPrefixPreparedCapacityForProfile(ctx, artifact, config, "numpy-core", []string{"numpy"})
	if err != nil {
		return nil, err
	}
	familyStarted := time.Now()
	imageConfig := config
	imageConfig.ExecutionProfile = &capacity.profile
	...
	bindings := numpycodec.Bindings{
		ArtifactSHA256: capacity.profile.ArtifactSHA256(), ExecutionProfileID: "numpy-core", ExecutionProfileSHA256: profileSHA256,
		ImportClosureSHA256: agentfunction.ImportClosureIdentity([]string{"numpy"}, []string{"numpy"}),
		SourceSHA256:        testDigest("gitchameleon-numpy-prepared-source"), InputsSHA256: testDigest("gitchameleon-numpy-prepared-input"), PassRegistrationSHA256: testDigest("gitchameleon-numpy-prepared-pass"),
	}
	producer, err := json.Marshal(numpycodec.ProducerValue{
		SchemaVersion: numpycodec.ProducerValueSchemaVersion, DType: "<i8", Shape: []uint64{1}, Order: "C", CContiguous: true,
	...
	input, err := wazeroengine.NewPreparedNumpyInput("dataset", descriptor, decoded)
	if err != nil {
		_ = capacity.Close(context.Background())
		return nil, err
	}
	for _, familyConsumers := range gitChameleonPreparedFamilyPartitions(maxConsumers) {
		family, familyErr := wazeroengine.PrepareNumpyFamily(ctx, artifact, wazeroengine.PreparedFamilyConfig{
			ImageConfig: imageConfig, MaxConsumers: familyConsumers, MaxActive: 1, Mode: wazeroengine.PreparedFamilyAuto,
		}, input)
		if familyErr != nil {
			_ = capacity.Close(context.Background())
			return nil, familyErr
		}
		capacity.executionFamilies = append(capacity.executionFamilies, family)
		capacity.executionFamilyLimits = append(capacity.executionFamilyLimits, familyConsumers)
	}
	capacity.setupNanos += uint64(time.Since(familyStarted))
	return capacity, nil
}

func gitChameleonPreparedFamilyPartitions(consumers uint32) []uint32 {
	const maxFamilyConsumers = uint32(1024)
	partitions := make([]uint32, 0, (consumers+maxFamilyConsumers-1)/maxFamilyConsumers)
	for remaining := consumers; remaining > 0; {
		familyConsumers := min(remaining, maxFamilyConsumers)
		partitions = append(partitions, familyConsumers)
		remaining -= familyConsumers
	}
	return partitions
}

func TestGitChameleonNumPyDerivedPreparedFamilyPartitions(t *testing.T) {
	got := gitChameleonPreparedFamilyPartitions(1200)
	if len(got) != 2 || got[0] != 1024 || got[1] != 176 {
...
func TestGitChameleonNumPyDerivedPLMFixture(t *testing.T) {
	...
	maxConsumers := uint32(manifest.TaskCount * len(manifest.MockStream.RatesTokensPerSecond) * runs * 2)
	capacity, err := newGitChameleonNumPyPreparedCapacity(context.Background(), artifact, config, maxConsumers)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
				...
				for order := 0; order < len(treatments); order++ {
					...
					case "serial_whole_file":
						...
						treatment, err = semanticspeculation.NewSerialGuestTreatment(semanticspeculation.SerialGuestTreatmentConfig{
							...
							RunnerFactory: func(ctx context.Context, manager *workspace.Manager, ref workspace.Ref) (enginecontract.Runner, *capability.Broker, error) {
								var broker *capability.Broker
								runner, runnerErr := capacity.newExecutionRunner(ctx, wazeroengine.PreparedRunnerConfig{
									RunConfig: runConfig,
									BrokerFactory: func(inner context.Context) (*capability.Broker, error) {
										var brokerErr error
										broker, brokerErr = brokerFactory(inner)
										return broker, brokerErr
									},
									Plan: plan, WorkspaceManager: manager, WorkspaceRef: ref, WorkspaceOwner: runID,
									InvocationRef: runtimeconfig.InvocationRef{AgentRunID: runID, InvocationID: runID, InvocationAttempt: 1, ExecutionID: runID},
								})
								return runner, broker, runnerErr
							},
						})
