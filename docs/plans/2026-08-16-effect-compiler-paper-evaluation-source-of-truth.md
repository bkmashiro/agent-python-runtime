...
**T2 optimization status (diagnostic, not a speedup claim):** remediation-v2 selected ordinary fresh execution plus programmatic tool calling; semantic analysis ran as a separate pre-pass. Each of the 36 logical READs created and closed one fresh analyzer Engine/Guest and then one fresh execution Engine/Guest, including fresh module compilation in both phases. Prepared runtime, memory COW, function cache, single-flight, semantic reuse and cold-I/O continuation were not selected; 0 runtime optimization events and 0 runtime observation events were recorded. The 36 Guest responses contain 36 capability calls and 48,238.338 ms aggregate `guest_time_ms` (median 1,406.124 ms; range 1,113.441–1,506.063 ms), but this timer starts immediately before `exec(code)` inside an already-started Guest and excludes source validation, interpreter/runtime construction, Wasm compilation/instantiation and outer Go orchestration. There is no matched no-Pysolate timing lane, so these values cannot establish overhead or speedup. T2-v1 direct recorded 83 tool calls and 149 provider calls, while the separate v2 treatment recorded 36 tool calls and 241 provider calls; the protocols, tool surfaces, failure modes and run dates differ, so neither the 56.6% lower tool-call count nor the 61.7% higher provider-call count is a causal optimization estimate.

**Future zero-model replay:** retain remediation-v2 unchanged and preregister a separate replay over its 36 frozen programs; use the workload-sensitive matrix in `docs/plans/2026-08-16-runtime-optimization-workload-matrix.md` rather than extrapolating from one READ. Compare a no-Pysolate direct-adapter control, ordinary fresh Pysolate, prepared-runtime Pysolate and memory-COW Pysolate only where admission selects the mechanism; treat the direct control as absolute-only because it bypasses the Guest/authority lifecycle, and run COW on a supported Linux host. Give every variant a frozen `variant_id`/configuration digest, include it in fresh Run/call identity, and use an independent evidence root. Require identical task/source/Plan/action and response/oracle digests, an explicit `observe.Session`, recorded `MechanismSet`/`PreparedState`/`COWProbe`, randomized lane order, warmup excluded by preregistration, repeated measured trials and separate Host wall-clock versus Guest `exec` timing. The corpus has 30 unique source/action pairs but 35 unique source+Plan identities, so only one event is an exact plan-bound duplicate. Whole-Run semantic reuse/function-result caching must remain excluded for these external READs unless a separate freshness/playback contract proves reuse safe. A one-program macOS fresh/prepared pilot pre-registered a frozen replay corpus identity and validated source/Plan/content digests, distinct variant-bound Run identities, six Host observation events per run and actual prepared selection; it is private and classified `HARNESS_VALIDATION_ONLY_NOT_PERFORMANCE_EVIDENCE`, so its timings support no performance claim.

**Post-fix replay-harness supersession:** pilot v10 is bound to a fixed public anchor, full case/config digests, and descriptor-rooted I/O with pre/post-open identity checks. It remains non-performance evidence.
