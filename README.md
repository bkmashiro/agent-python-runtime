...
**Current artifact-bound profile admission:** an optional untrusted `compatibility` manifest names `base` or `numpy-core` and declares imports. Host policy freezes allowed import roots. The schema-v3 build pipeline runs `importlib.find_spec` inside the exact target Guest over builtin, stdlib, and packaged roots; it emits a checksummed `import-inventory.json`, embeds the same sorted inventory in the distribution manifest, and the canonical verifier binds artifact, manifest, sidecar, profile, and package identity before Factory/Guest work. Host `allowed_imports` must be a subset of those Guest-discoverable roots. CLI, Hermes bridge, and Wazero reject identity, sidecar, inventory, or policy mismatches. Schema-v2 bundles remain readable as legacy artifact identity but cannot enable profile binding. This is not arbitrary-Python source or import-execution proof: `find_spec` discoverability does not prove module initialization, transitive imports, native behavior, or Run success. See [Execution profile admission](docs/profile-admission.md).
