...
def transform(source, manifest):
    ...
    tool_names = {spec["name"] for spec in manifest}
    ...
    def tool_call(node):
        return (isinstance(node, ast.Call) and isinstance(node.func, ast.Name)
                and node.func.id in tool_names and not node.args
                and all(k.arg is not None and argument(k.value) for k in node.keywords))

    def early_call(node):
        return tool_call(node) and node.func.id in early_names

    def block_ok(body):
        for s in body:
            if isinstance(s, ast.Assign):
                if (len(s.targets) != 1 or not isinstance(s.targets[0], ast.Name)
                        or s.targets[0].id == "inputs" or s.targets[0].id in tool_names
                ...
                if not all(block_ok(b) for b in (s.body, s.orelse, s.finalbody)):
                    return False
                for h in s.handlers:
                    if h.name == "inputs" or h.name in tool_names or not block_ok(h.body):
                        return False
            ...
            if isinstance(s, ast.Assign) and early_call(s.value):
                ...
                for index, statement in enumerate(group):
                    ...
                    statement.value = ast.copy_location(ast.Call(ast.Name(resolve, ast.Load()),
                        [ast.Name(slot, ast.Load()), ast.Constant(call.func.id)], call.keywords), call)
