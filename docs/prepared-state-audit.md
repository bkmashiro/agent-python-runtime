...
Capacity above zero remains opt-in because each queued exact-artifact candidate retains at least 128 MiB and refill adds substantial initialization CPU work. Same-commit Linux evidence for the audited base strategy records 7.492s factory-to-ready, 3.843ms first execute, 1.854ms steady execute, 26.983ms prepared capability median, and 128 MiB queued Guest linear memory; these are descriptive measurements without a threshold or process-RSS claim. The later explicit `numpy-core` candidate independently records the same lifecycle/resource fields under `profile-candidate`. No prepared-restore mode or linear-memory copy strategy was implemented; capacity zero remains the default.
