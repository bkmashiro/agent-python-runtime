...
The next experiment therefore rebuilds only the pack CLI from the same locked wasi-vfs source and injects a fixed pack-only monotonic clock (`resolution = 1 ns`, `now = 0`) while preserving all previous producer cleanups. A local Linux preflight against the retained raw Wasm and manifest-identical VFS produced byte-identical Wasm from two independent CLI processes. This is strong same-input evidence, but the hypothesis remains provisional until the clean dual-builder artifacts and full-bundle exact gate agree. Production runtime clocks remain untouched.
