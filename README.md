...
**Current artifact-bound profile admission and source comparison:** an optional untrusted `compatibility` manifest names `base` or `numpy-core` and declares imports. Host policy freezes allowed import roots. The schema-v4 build pipeline first runs `importlib.find_spec` inside the exact target Guest and emits checksummed `import-inventory.json`; it then runs a curated set of imports and deterministic operations in separate fresh Guest invocations and emits `import-qualification.json`. The canonical verifier binds artifact, manifest, both sidecars, profile, and package identity before Factory/Guest work. Host `allowed_imports` must be a subset of the qualified roots, which must themselves be a subset of the discoverable roots. `conservative-python-imports-v1` additionally compares bounded obvious static imports in exact request source against caller declarations and the Host/profile set before Guest work. Dynamic import/execution, relative imports, lexical ambiguity, and comparison bounds overflow are `indeterminate` and fail closed. The immutable Host result binds exact source, profile, and—after profile binding—artifact/manifest digests while recording `syntax_checked=false`. Schema-v2/v3 bundles remain readable as legacy artifact identity but cannot enable profile binding. This is not exact Python parsing, transitive closure, arbitrary module behavior, or Run-success proof. See [Execution profile admission](docs/profile-admission.md) and [Conservative source import comparison](docs/source-compatibility.md).
