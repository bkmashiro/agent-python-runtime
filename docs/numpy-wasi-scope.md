...
Remaining promotion work is policy rather than an unverified build claim:

- decide whether the explicit `numpy-core` candidate should remain manual-only or receive a recurring non-default gate;
- add profile-aware performance evidence before making any latency/retained-memory promise for the normal artifact path;
