...
class NumPyWASIProbeContractTests(unittest.TestCase):
    def test_probe_uses_isolated_exact_sources_and_bounded_features(self):
        ...
        self.assertIn('"durations_ns"', script)
        self.assertIn("time.time_ns()", script)
        self.assertNotIn("time.monotonic_ns()", script)
        self.assertIn('"total"', script)
        ...
        self.assertIn("extension-archives.txt", script)
        self.assertIn("static-inputs.txt", script)
        self.assertIn('-Wl,--whole-archive "${NUMPY_EXTENSION_ARCHIVES[@]}" -Wl,--no-whole-archive', script)
        self.assertNotIn('--whole-archive "${NUMPY_LINK_INPUTS[@]}"', script)
        ...
        self.assertIn('export_name("numpy_random_probe")', link_source)
        self.assertIn('export_name("numpy_entropy_probe")', link_source)
        self.assertIn('export_name("numpy_freshness_probe")', link_source)
        self.assertIn('export_name("numpy_freshness_word_probe")', link_source)
        self.assertIn("_numpy_wasi_fresh_marker", link_source)
        self.assertIn("default_rng(123456789)", link_source)
        self.assertEqual(link_source.count("default_rng()"), 2)
        ...
        self.assertIn('ExportedFunction("numpy_entropy_probe")', verifier_source)
        self.assertIn('callSingleResult(ctx, module, "numpy_freshness_probe")', verifier_source)
        self.assertIn('callSingleResult(ctx, module, "numpy_freshness_word_probe")', verifier_source)
        self.assertIn("FreshInstancesValidated", verifier_source)
        self.assertIn("CompileDurationNS", verifier_source)
        self.assertIn("MemoryBytes", verifier_source)
        self.assertIn("HostHeapAllocBytes", verifier_source)
        self.assertIn("WithRandSource(cryptorand.Reader)", verifier_source)
        self.assertIn('"host_crypto_rand"', verifier_source)
        self.assertIn('"numeric_succeeded"', verifier_source)
        self.assertIn('"random_succeeded"', verifier_source)
        self.assertIn('"entropy_succeeded"', verifier_source)
        self.assertIn('"freshness_succeeded"', verifier_source)
    ...
    def test_workflow_is_manual_only_and_uploads_diagnostic_evidence(self):
        ...
        self.assertIn("feature_profile:", text)
        self.assertIn("- core", text)
        self.assertIn("- random", text)
        self.assertIn("NUMPY_WASI_FEATURE_PROFILE: ${{ inputs.feature_profile }}", text)
        self.assertIn('-profile "${{ inputs.feature_profile }}"', text)
