...
Status: design rationale and historical evidence. The previous execution plan at `docs/plans/2026-08-20-semantic-speculation-optimizer-autonomous-megagoal.md` is paused. Proposed successor work is governed by `docs/plans/2026-08-27-unified-split-phase-execution-roadmap.md`. No executable region consumer is enabled by this document.
...
**2026-08-21 package-profile decision:** Phase 0 froze the NumPy reuse case matrix and economics contract before timing. Phase 1 replaced the pure-Python package special case with a closed repository profile registry. Phase 2 qualified the first Pysolate-native static package artifact: NumPy `1.26.0b1` is built from locked source against Pysolate CPython `3.14.0` and wasi-sdk `33`, 19 native modules and two support archives are statically linked into the normal `runtime.c` reactor, and exact Guest inventory/operation probes pass under the `numpy-core` artifact profile. Two fresh Host-bound runs produced identical results with zero capability calls; a filesystem attempt published no result, and `base` substitution failed before Guest execution. Phase 3 moved the two existing semantic consumers onto one closed registration contract without changing their projections or execution authority. Phase 4 added a run-scoped Host-owned immutable blob and one-shot lease lifecycle; independent review findings on codec length, pre-parse metadata bounds and 32-bit count comparisons were fixed and the 32-bit regression passed as a real Linux/386 binary. Phase 5 now supports only bounded C-contiguous numeric ndarray bodies with exact dtype, shape, order, endianness, body, artifact, profile, source, input, pass, consumer and final-source joins. Formal P5 macOS and Linux evidence uses the original Phase 2 artifact; each platform passed matched recompute/result and error/log parity, mutation isolation and zero capability calls, while all seven Linux P5 Guests used private-COW with no fallback. Phase 6 closes a deliberately small Host-generated producer language: `arange_affine_i64_v1` supports the mechanism oracle, while `zeros_f64_v1`, `affine_f64_v1`, `sum_i64_v1`, and `matmul_f64_v1` cover exactly the frozen campaign compute classes. Exported admission accepts only opaque analysis minted by the concrete exact-Guest Wazero analyzer; raw or fabricated analysis values cannot cross that boundary. Conservative unknown native-call effects remain recorded as unknown and are not described as DBI or a purity certificate. Publication requires a second opaque proof minted by a concrete Wazero execution with stable pre/post artifact/profile properties, no mounted workspace or capability broker, successful terminal response, zero capability calls and exact source binding. Prepared lineage reconstructs the expected decision binding and joins consumer, final source, inputs and request identities. Random/time/file/dynamic-import/object/unknown-call, profile, stale-source and input substitution remain rejected. The Guest execution request and response buffers match the existing 16 MiB Host hard bound so the frozen 8 MiB body cell can pass its base64/JSON envelopes; tool-response and prepared-scalar bounds remain unchanged. The replacement immutable artifact is `sha256:2753cde560f3961a483df53aec334c8fdbb084934e5a62a56d436aea1ae557ad`, built from `fac8a8798f5f97f28c42962b217e2f79c2a985a5`; prior artifacts remain immutable. Post-review macOS and Linux P6 evidence covers all five positive declarations and six Exact Guest adversarial sources, with every Linux analysis and execution using private-COW without fallback. This establishes package, typed transport and narrow producer-admission mechanics only. It does not promote arbitrary NumPy purity or establish result-reuse economics. P7 restarts from zero only after the fixed-target independent review passes.

**2026-08-21 prepared-data decision:** **Current:** a research-only `.npy v1` lane requires one exact target-Guest `numpy.load` candidate plus a separate Host-authored `sources.read` contract. Physical read, strict bounded Go decode and Run-private seal are separately observable. The fixed post-closeout path binds preparation, contract, body, profile, imports and Plan into the prepared-region decision; one Host call atomically consumes its dynamic token and exact staged object, while Guest-side SHA-256 verification prevents a substituted copied body from acquiring logical effect. Source/argument drift, unreached branches, rejection and cancellation retain the original fresh-Guest execution path and leave speculative physical work unclaimed. **Contribution boundary:** private-COW and prepared NumPy package shards are prior Pysolate substrate, used here to avoid making fresh isolation dominate the experiment. The prepared-data contribution is operation-granular authority-preserving pre-dispatch: only an explicitly qualified immutable physical effect can move early, its logical `numpy.load` remains at the exact dynamic occurrence, and invalid speculation can be discarded rather than forcing the persistent-interpreter authority model used by the EAGER control. **Measured:** macOS and Linux real-NumPy probes pass the fixed object/token join, one-shot/replay, body identity, numerical parity and cleanup checks. The fixed fanout probes establish parity, mutation isolation and copy/map accounting. The Phase 6 checker reconstructs 162 coordinates and 54 aggregates from three raw trial pairs; critical paths are schedule-model projections from measured components, sufficient for mechanism evaluation but not presented as production wall-clock attribution. **Rejected:** generic object stores, arbitrary Host Python, persistent heap snapshots, automatic optimizer/API promotion and universal speedup claims. CPython package preparation retains WASI clock/random and is not described as authority-free. **Deferred outside the paper:** larger fixtures, generic loaders, cross-Run retention and single-flight. Observed interval tracing is intentionally not a paper deliverable.
...
**2026-08-20 coordinate lifecycle:** A deterministic runner now fixes the four economics cells before the actual adapter exists. It creates one final capacity for original and analyzer/scratch/final capacities for derived, placing them before or after clock start according to profile. The finalization gap overlaps cold provisioning and the derived preparation pipeline; final validation, compile/load and execution occur after it. The adapter-facing input contains only unchanged source, focus index and output name, excluding coordinate labels and expected outcomes. Failures drain the gap and tear down without producing evidence. Fake-clock tests prove exact call order, interval dispositions, canonical record acceptance and duplicate-safe JSONL across all four cells. These are synthetic control records only; the reviewer still reports zero campaign timing observations. Exact Guest adapter wiring and a sealed harness identity remain next.

**2026-08-20 late table publication:** A pre-provisioned final Guest can now exist before its scratch result does. Its Engine receives an empty run-private table; later, the Host may publish exactly one valid decision-bound capsule into a missing or matching unready entry. Publication cannot overwrite ready/consumed/discarded state and rejects invalid, repeated or closed-table attempts. Exact macOS and Linux tests provisioned final capacity first, proved the table remained empty, published the capsule, and then completed fresh derived execution with one claim and result `42`; Linux required private COW. This removes the final-capacity ordering blocker without adding authority, fallback or a generic transport. No campaign timing was observed.

**2026-08-20 split final stages:** One initialized, never-served final capacity now spans two explicit one-shot operations. `Compile` performs Host binding checks, unchanged-source validation and target-Guest derived selection compile/load without execution or capsule claim. `Execute` accepts only the byte-identical request and invokes formal execution once before terminal close. Both stages report zero module instantiations and zero `runtime_init` calls because provisioning owns those costs; Linux provisioning must produce a private COW hit. Invalid suffix compile closes the capacity with zero claim, and repeated execution rejects rather than replaying or falling back. Exact macOS, race and Linux tests retain result `42`. This lets the frozen recorder measure final compile/load separately from formal execution. No campaign timing was observed.
...
**2026-08-21 P5R mechanism closure:** The original frozen P5 study remains a no-go with five mechanism failures and zero timing observations; its 80-record economics campaign was never started. A separately preregistered remediation retained the same 11 cases and authority/freshness envelope, replaced only demonstrated recursive AST work with bounded iterator-frame/post-order logic, fixed terminal discard projection and bound the emitted final AST back to the analyzed decision. Reviewed artifact `sha256:62454f9689ae4a11e45d51398e1d605be91b58b472eaafc26a994cb5669f62e9` and harness `sha256:ea88c6f2de16e5932c5211158b94c9d1efaa5d598096a0643def9276e153d042` passed 11/11 on macOS and Linux private COW with no Broker/workspace authority, replay or transport expansion. The representative body-free cost profile remained negative: original versus derived critical wall was 3.659 versus 11.078 seconds on macOS and 12.093 versus 53.366 seconds on Linux, dominated by provisioning three fresh capacities. This cost shape is reported as an applicability constraint, not a mechanism-retention or Phase 6 gate.

**Gate R4 (historical economics criterion): not used for P5R promotion.** Baseline and patched lanes now match result, exception class, logical calls, authority state and terminal capsule lifecycle across the frozen positive and adversarial mechanism cases. Positive net latency was not observed and is no longer required for this paper-closure lane. Any Phase 6 work requires a separate architecture decision; this closure does not authorize generic typed transport.
...
**2026-08-21 bounded NumPy lane:** R6 has one closed mechanism rather than a generic codec: the Pysolate-owned, source-locked `numpy-core` CPython/WASI artifact and `numpy_ndarray_c_v1` transport accept only bounded little-endian numeric C-contiguous arrays. A verified concrete-Wazero producer execution yields authority bound to the exact raw result, run, artifact/profile/source/input/pass joins and body budget; the sole Host decode then seals the exact run/codec/canonical-metadata/descriptor/body identity before Store publication. A copied authority cannot authorize different raw bytes, bindings, run, budget, metadata, body or descriptor. Each consumer receives one private materialisation copy in a fresh Guest; no Python heap, pointer, allocator, workspace or capability state crosses the boundary. Native NumPy calls remain conservatively unknown because this is Host-side WASI/runtime/capability/lifecycle evidence, not instruction-level DBI or a purity certificate. Independent Phase 6 review returned `FINAL: PASS blockers=0`.

The preregistered campaign then completed 240 records, 80 treatment cells and 40 matched economic comparisons across macOS and Linux. All mechanism and lifecycle checks passed, including Linux preprovisioned private-COW with zero fallback, but reuse lost in all 40 observed cells. The best ratio was `0.7952187480012893` for the macOS preprovisioned four-consumer matrix case with a 45-second lead gap; its net saved time was still `-16.673223291 s`. Independent compute, consumer-count and lead-gap thresholds remain `not_identified_from_coupled_sparse_grid`. Therefore the transport remains a bounded research capability, is not enabled as a performance default, and does not trigger a fan-out/single-flight follow-on. A future economic claim requires a materially different preregistered workload rather than interpolation over this grid.
