...
**Current artifact-bound profile admission:** an optional untrusted `compatibility` manifest names `base` or `numpy-core` and declares imports. Host policy freezes allowed import roots; the canonical artifact verifier binds that policy to the exact distribution profile, package set, artifact SHA-256, and manifest SHA-256 before Factory/Guest work. CLI, Hermes bridge, and Wazero reject identity or manifest mismatches. This is not arbitrary-Python source proof: the v2 manifest does not yet enumerate every import root supplied by CPython/WASI. See [Execution profile admission](docs/profile-admission.md).
