...
class NumPyWASIProbeContractTests(unittest.TestCase):
    def test_probe_uses_isolated_exact_sources_and_bounded_features(self):
        ...
        self.assertIn("_multiarray_umath.json", script)
        self.assertIn("_umath_linalg.json", script)
        self.assertIn("expected two selected extension archives plus exactly three unique static inputs", script)
        ...
        self.assertIn("initializer/import/numeric execution is reported separately", script)
        link_source = LINK_PROBE.read_text()
        self.assertIn("PyInit__multiarray_umath", link_source)
        self.assertIn("PyInit__umath_linalg", link_source)
        self.assertIn('PyImport_AppendInittab("numpy._core._multiarray_umath"', link_source)
        self.assertIn('PyImport_AppendInittab("numpy.linalg._umath_linalg"', link_source)
        self.assertNotIn('PyImport_AppendInittab("numpy.linalg.lapack_lite"', link_source)
