...
func (manager *Manager) CreateFromDirectory(source string, limits Limits) (Ref, error) {
	...
	ref, destination, err := manager.allocateLocked()
	if err != nil {
		return "", err
	}
	failed := true
...
func copySourceDirectory(source *os.Root, logicalPath, destination string, rootDevice uint64, limits Limits, usage *ingressUsage, hooks *ingressHooks) error {
	...
	for _, directoryEntry := range entries {
		...
		// Repository metadata remains Host-owned. Agent code edits the working
		// tree while Git inspection and publication happen outside the Guest.
		if name == ".git" {
			continue
		}
		relative := name
