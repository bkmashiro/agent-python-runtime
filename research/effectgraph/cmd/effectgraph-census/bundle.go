...
const evidenceBundleSchema = "pysolate.effectgraph-census-bundle.v0"
...
func encodeEvidenceBundle(corpus, effectReport, regionReport []byte, corpusIdentity, artifactSHA, artifactSourceCommit string) ([]byte, error) {
	bundle := evidenceBundle{
		SchemaVersion:    evidenceBundleSchema,
		CorpusFileSHA256: digest(corpus), EffectReportFileSHA256: digest(effectReport), RegionReportFileSHA256: digest(regionReport),
...
func validEvidenceBundle(bundle evidenceBundle) bool {
	return bundle.SchemaVersion == evidenceBundleSchema &&
		...
		validSHA256(bundle.RegionReportFileSHA256) && validSHA256(bundle.CorpusIdentitySHA256) &&
		validSHA256(bundle.ArtifactSHA256) && len(bundle.ArtifactSourceCommit) == 40 && isLowerHex(bundle.ArtifactSourceCommit)
}

func verifyEvidenceBundle(bundleJSON, corpus, effectReport, regionReport []byte) error {
	...
	if json.Unmarshal(bundleJSON, &bundle) != nil || !validEvidenceBundle(bundle) ||
		...
		bundle.RegionReportFileSHA256 != digest(regionReport) {
		return errors.New("evidence bundle mismatch")
	}
	return nil
}

func writeEvidenceBundle(bundlePath, corpusPath, effectReportPath, regionReportPath string, corpus, effectReport, regionReport []byte, corpusIdentity, artifactSHA, artifactSourceCommit string) error {
	bundle, err := encodeEvidenceBundle(corpus, effectReport, regionReport, corpusIdentity, artifactSHA, artifactSourceCommit)
	if err != nil {
		return err
	}
	...
	for _, output := range []struct {
		path    string
		content []byte
	}{{corpusPath, corpus}, {effectReportPath, effectReport}, {regionReportPath, regionReport}, {bundlePath, bundle}} {
...
func verifyEvidenceBundleFiles(bundlePath, corpusPath, effectReportPath, regionReportPath string) error {
	...
	return verifyEvidenceBundle(bundle, corpus, effectReport, regionReport)
}
