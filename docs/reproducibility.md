...
Run `29934585110` on signed candidate `833b854` proved that the narrow archive patch compiled, linked, passed producer ABI checks, and generated schema-valid supply-chain evidence in both replicas. Exact comparison still failed: the Wasm data sections differed by two bytes in total size and digest. Zeroing the directly evidenced C padding was therefore necessary evidence but insufficient to make the broader Wizer/CPython heap snapshot deterministic.

Exact reproducibility is **not claimed**. Another producer patch now requires an explicit architecture/resource decision: maintain a deeper pinned wasi-vfs/Wizer fork with comprehensive deterministic-memory ownership, replace Wizer-based VFS packaging, or defer the exact reproducibility promise. The comparator and acceptance rule remain unchanged.
