...
The shareable entry binds the actual request source, canonical inputs, output contract, and artifact import inventory to the invocation identity, and binds the created Runner to the bytes-derived artifact identity, execution profile, and deterministic profile. Its in-flight key is domain-separated from generic callback single-flight, and completed-result retention is deliberately rejected in this first prototype. Runners with a workspace or capability Broker are rejected before `Run`. A Host-call attempt remains unpublishable even when Guest Python catches the bridge error; a subsequent identical call recomputes rather than retaining the failed value. Child-specific filesystem materialization occurs afterward in separate Host-owned workspaces.
