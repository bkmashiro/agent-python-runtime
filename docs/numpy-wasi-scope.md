...
Only after F2 succeeds, first cross the initializer/import boundary in an experiment-only artifact. Run `29964718389` performed a real Meson install, deterministically staged 381 NumPy package files, packed the CPython standard library plus that tree, registered the core builtin, and entered its initializer under current wazero. Its first runtime trap was resolved with the locked SDK's own `libc-printscan-long-double.a`, preserving the measured 16-byte `long double` / `IEEE_QUAD_LE` target ABI instead of using the historical `--wrap=strtold` binary64 shim. Run `29965554161` then initialized Python and crossed that core initializer path; its next complete traceback showed top-level NumPy's eager matrix compatibility imports require a second builtin, `numpy.linalg._umath_linalg`. The next diagnostic links/registers only that manifest-bound archive and its deduplicated static inputs. This remains diagnostic until the runtime report records a completed top-level import.
