...
- `tools/phase6_slurm_watch.py` is the local controller. Every expected Host, tree, validator and Guest artifact identity is an explicit argument. It opens each remote archive/control file once with `O_NOFOLLOW | O_NONBLOCK`, rejects non-regular files before streaming, enforces a total transfer deadline, applies a second local byte bound, rejects unsafe or duplicate archive members and duplicate JSON keys, requires the exact canonical cell/repetition/evidence set for the selected tier, verifies the worker environment and final requested/allocated Slurm resource shape, separately binds the staged Linux benchmark binary and the controller-local exact validator by SHA-256, verifies every evidence checksum with that validator, and only then publishes the archive-hash ACK.
