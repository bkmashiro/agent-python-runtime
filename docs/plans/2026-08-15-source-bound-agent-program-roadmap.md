...
**Status:** Bounded dataset-first pilot, natural task-oracle spike and one exact artifact-bound package profile complete on 2026-08-16. Fifty CodeAct records and ten mixed-language Open-SWE trajectories produced a body-safe deterministic census; eight naturally occurring Python actions completed under local CPython and the exact Host-profile-bound real Guest. The pinned `attrs-770` work then reproduced semantic RED/GREEN, built a provenance-bound pure-Python Guest artifact, qualified the named operation in the restricted Agent-body phase, and passed the profile-bound oracle twice. The profile remains non-default and does not authorize a generic package system, sharing pass, scheduler or worker pool.
