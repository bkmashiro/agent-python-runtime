...
if [[ $artifact_profile == attrs-770 ]]; then
  ssh shell2 mkdir -p "$stage/private"
  scp -q "$extension_patch" "shell2:$stage/private/extension.patch"
  ssh shell2 chmod 0700 "$stage/private"
  ssh shell2 chmod 0600 "$stage/private/extension.patch"
...
ssh shell2 ssh gpu31 env AGENT_RUNTIME_ARTIFACT_PROFILE="$artifact_profile" AGENT_RUNTIME_EXTENSION_PATCH="$remote_patch" \
