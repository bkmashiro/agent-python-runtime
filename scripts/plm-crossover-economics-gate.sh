...
ZERO_READ_RUNS=${PLM_CROSSOVER_ZERO_READ_RUNS:-1}
...
ZERO_ONLY=${PLM_CROSSOVER_ZERO_ONLY:-0}
...
if [[ ! "${ZERO_READ_RUNS}" =~ ^[0-9]+$ ]] || (( ZERO_READ_RUNS < 1 || ZERO_READ_RUNS > 20 )); then
  echo "PLM_CROSSOVER_ZERO_READ_RUNS must be in [1,20]" >&2
  exit 2
fi
if [[ ! "${ZERO_ONLY}" =~ ^(0|1|false|true)$ ]]; then
  echo "PLM_CROSSOVER_ZERO_ONLY must be a boolean" >&2
  exit 2
fi
