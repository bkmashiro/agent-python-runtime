...
The verified read-only production CLI now creates one Host-owned workflow transaction per capability-bearing Run. The legacy `fetch_many` Guest API is adapted into a sealed Registry handler and every admitted call is journaled as an operation+attempt; an optional absolute `transaction_journal_path` enables SQLite/WAL reopen evidence. Generated Python exposes only `current_transaction()` as an opaque `{scope, authority, lifecycle, catalog_digest}` projection—no transaction IDs, undo data, approval, rollback, compensation, or commit authority. Host response validation enforces the requested output schema before workflow commit, and unhandled failure drives bounded Host abort. Reversible/compensatable writes remain local fixture adapters rather than production-provider claims.
