...
func TestDirectPreparedNumpySumMatchedEndToEnd(t *testing.T) {
	...
	for index := 0; index < 5; index++ {
		baselineRequest := numpySumRunRequest(t, fmt.Sprintf("direct-value-slot-baseline-%d", index))
		...
		runBaseline := func() {
			...
			assertNumpySumResult(t, baselineRequest, response, runErr)
		}
		runTreatment := func() {
			started := time.Now()
...
func numpySumRunRequest(t *testing.T, runID string) runtimeconfig.RunRequest {
	t.Helper()
	return runtimeconfig.RunRequest{
		RunID: runID, Code: numpySumSource, Inputs: json.RawMessage(`{}`),
...
func assertNumpySumResult(t *testing.T, request runtimeconfig.RunRequest, response []byte, runErr error) {
	t.Helper()
	if runErr != nil {
		t.Fatal(runErr)
	}
