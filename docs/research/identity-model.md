...
| capability Plan identity | Host hashes canonical `pysolate.capability-plan.v6` containing sorted Specs, Grant bindings and max calls | Broker, receipts, response projection, Playback admission | Authority root | Recomputable from the full sealed Plan; Host response may expose it as evidence |
